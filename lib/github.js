...
module.exports.invoke = async function githubProvider(opts) {
	...
	const res = await http.header('User-Agent', 'strugee/count-your-issues')
	                      .get('https://github.com/search?utf8=%E2%9C%93&q=author%3A' + opts.username + '+is%3Aissue&type=Issues');

	const document = new JSDOM(res.data).window.document;

	// The .replace() strips all non-number characters
	// TODO assert some stuff to make sure GitHub doesn't (silently) break this when they change HTML
